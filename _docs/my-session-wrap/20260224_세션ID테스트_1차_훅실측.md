...
1. 현재 세션 종료 (`/exit` 또는 Ctrl+C) — SessionStart는 새 세션 시작 시에만 발동
2. 새 세션 시작 → SessionStart hook 발동 → `/tmp/claude-hook-test-sessionstart.json` 생성 예상
3. 프롬프트 입력 → UserPromptSubmit 발동, 응답 완료 후 Stop 발동
...
> **⚠️ 정정 (2차·3차 테스트에서 확인)**
>
> 위 "stdin 공유" 가설은 **오결론**이었다. 2차 테스트(`세션ID테스트_2차_Node전환_20260224.md`)에서
