...
> Hook을 작성·수정할 때 아래 규칙을 반드시 따르라. 위반 시 "조용한 실패"로 디버깅이 극히 어렵다.

| # | 규칙 | 근거 |
|---|------|------|
| 1 | **hooks.json은 이중 배열 구조**: `"Event": [{ hooks: [{ type, command }] }]`. 내부 `hooks` 배열을 빠뜨리면 조용히 미실행 | §2 |
| 2 | **Hook 스크립트는 Node.js(.js)로만 작성**. bash/python은 Windows에서 실행 불가 (`cmd.exe`가 shebang 미해석) | §8 |
| 3 | **이벤트명은 PascalCase**: `SessionStart`, `UserPromptSubmit`, `Stop`, `PreToolUse`, `PostToolUse`. 대소문자 틀리면 조용히 무시 | §1 |
| 4 | **stdin은 반드시 방어**: `if (!input) return` + `try-catch`. 빈 stdin에서 `JSON.parse("")` crash 방지 | §3 |
| 5 | **파일 저장은 `cwd` 기준**: stdin의 `cwd` 사용. 글로벌 경로(`~/.claude/`)에 저장하면 동시 세션 충돌 | §4 |
| 6 | **timeout 명시** (권장 3000ms). 10ms는 I/O에 너무 짧아 실패함 | §8 |
| 7 | **hook 실패가 세션을 차단하면 안 됨**: `process.exit(1)` 지양, 에러는 조용히 삼킬 것 | §7 |
| 8 | **커맨드 파일에 `plugin: <name>` 마커 필수** (콜론 뒤 공백 포함). 충돌 감지의 핵심 | §5 |
...
hook이 `console.log()`로 출력한 내용은 **사용자 터미널 화면에 표시되지 않는다.**
대신 **Claude의 컨텍스트(system-reminder)에 주입**된다.

```
hook stdout → Claude 컨텍스트(system-reminder) ← Claude만 볼 수 있음
           ✗ 터미널 UI (사용자 화면에 미표시)
```

이 특성을 활용하면 세션 시작 시 Claude에게 절차·규칙을 강제 주입할 수 있다.

**검증법**: 새 세션에서 Claude에게 직접 질문
```
방금 세션 시작 시 훅으로 받은 내용이 있는가?
```
Claude가 hook stdout 내용을 답변에 포함하면 정상 동작.

> **⚠️ Common Error: 터미널에서 출력 확인 시도**
> hook stdout이 터미널에 안 보인다고 "동작 안 함"으로 오해하기 쉽다.
> 실행 여부는 파일 생성으로, 출력 내용은 Claude에게 질문으로 확인하라.
