...
### 기존 SessionStart hook 실패 원인 확정

이전 세션에서 플러그인 hooks.json의 `capture-session-id.sh`가 디버그 로그조차 생성하지 못한 원인:

```
"matcher": "startup",
"hooks": [
  { "command": "node ensure-commands.js" },   ← stdin JSON을 소비
  { "command": "bash capture-session-id.sh" }  ← 빈 stdin 수신 → cat이 블록/즉시 종료
]
```

**같은 hooks 배열 내 복수 hook은 stdin을 공유하며, 첫 번째 hook이 stdin을 소비하면 이후 hook은 빈 데이터를 받는다.** 이번 글로벌 settings.json 테스트에서는 SessionStart hook이 단독으로 실행되어 정상 동작했다.
